...
@section("auth-contents")
    ...
    @if(session('error'))
        <p class="my-10 text-center border border-red-400
        bg-red-200 py-3 text-sm uppercase">{{session('error')}}</p>
    @endif


    <form class="mt-14 space-y-5" novalidate action="{{route('login.store')}}" method="post">
    @csrf
